...
class ApiController extends CoreController {
    ...
    public function indexAction() {
        ...
        $aReturn = ['state'=>'success','message'=>'Welcome to onePlace Task API'];
    ...
    public function listAction() {
        ...
        if(!$this->checkLicense('task')) {
            $aReturn = ['state'=>'error','message'=>'no valid license for task found'];
            echo json_encode($aReturn);
            return false;
        }
        ...
        foreach($aLangs as $sLoadLang) {
            if(file_exists(__DIR__.'/../../../oneplace-translation/language/'.$sLoadLang.'.mo')) {
                $translator->addTranslationFile('gettext', __DIR__.'/../../../oneplace-translation/language/'.$sLang.'.mo', 'task', $sLoadLang);
            }
        }
        ...
        $aFields = $this->getFormFields('task-single');
        $aFieldsByKey = [];
        ...
        # Get All Task Entities from Database
                        ...
                        switch($oField->type) {
                            case 'multiselect':
                                ...
                                foreach($oTags as $oTag) {
                                    $aTags[] = ['id'=>$oTag->id,'label'=>$translator->translate($oTag->text,'task',$sLang)];
                                }
                            ...
                            case 'select':
                                ...
                                if($oTag) {
                                    if (property_exists($oTag, 'tag_value')) {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->id, 'label' => $translator->translate($oTag->tag_value,'task',$sLang)];
                                    } else {
                                        $aPublicItem[$oField->fieldkey] = ['id' => $oTag->getID(), 'label' => $translator->translate($oTag->getLabel(),'task',$sLang)];
                                    }
                                }
                                break;
                            case 'text':
                            case 'date':
                            case 'textarea':
                                $aPublicItem[$oField->fieldkey] = $translator->translate($oItem->getTextField($oField->fieldkey),'task',$sLang);
                                break;
                            default:
                                break;
                        }
    ...
    public function getAction() {
        ...
        try {
            ...
            $aReturn = ['state'=>'error','message'=>'Task not found','oItem'=>[]];
            echo json_encode($aReturn);
            return false;
        }

        # Print Entity
        $aReturn = ['state'=>'success','message'=>'Task found','oItem'=>$oItem];
        echo json_encode($aReturn);
